...
///
/// A packet's construction list holds what it declared; its resource list
/// holds what it uses, which an earlier packet may have declared. Both need
/// materializing.
///
/// A texture view is neither, and saying so is the point of the third arm: a
/// view is declared carrying its base's storage, so it answers the
/// linear-allocation question exactly as well as its base does and would
/// otherwise be claimed by the arm ahead of it and built as an allocation of
/// its own. It has no allocation and no image; it is installed onto its
/// owner's image when it is declared.
...
pub(crate) fn fnv_discriminant(reason: &str) -> u64 {
...
#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct ReplacementCoordinatorDiagnostic {
                ...
                self.runtime.execution_representation_coverage(backing)
            {
                crate::observe::off(format!(
                    "replacement_stale_representation backing={} view={view:?} \
                     representation={} route={route:?} holds=[{holds}]",
                    backing.get(),
                    representation.get()
                ));
            }
...
mod tests {
    ...
    /// Declaring a view is what installs it, whenever the image it belongs on
    /// already exists.
    ///
    /// The other order needs nothing, because a texture materialized after its
    /// views were declared reads them and carries them. Doing it anywhere else
    /// means knowing which resources a later packet will name, and a sampled
    /// compute texture is reached through the object table and named nowhere a
    /// materialization pass can see -- which is where this was lost, as
    /// `UnknownImageView { ShaderViewAbsent }` against an otherwise correct
    /// image, with no install refusal anywhere in the log because no install
    /// had ever been attempted.
    #[test]
    fn a_view_declared_after_its_owners_image_is_installed_onto_it() {
        use crate::runtime::host::HostMemory;
        use reims_vgpu_paging::geometry::{DIRECTORY_DEPTH, DIRECTORY_ROOT_PFN};
        ...
        let semantic = runtime
            ...
            .resources()
            .graph()
            .resolve_texture_binding_view(view.resource)
            .unwrap();
        ...
        let native = runtime
            .execution()
            .resources()
            .representation(base.backing, representation)
            .unwrap();
        assert!(
            native.shader_view(semantic).is_ok(),
            "the view was declared onto an image that already existed, so it \
             has to be on it"
        );
    }

    /// A texture view is declared carrying its base's storage, so it answers
    /// the linear-allocation question as well as its base does. Classified as
    /// one, it would be built as an allocation of its own -- and it has none.
    #[test]
    fn a_texture_view_an_exec_names_is_not_classified_as_an_allocation() {
        let Some(mut runtime) = runtime() else {
            return;
        };
        let shift = crate::model::PAGE_SHIFT_X86;
        ...
        assert!(
            linears.contains(&base.resource),
            "the texture is the allocation: {linears:?}"
        );
        assert!(
            !linears.contains(&view.resource),
            "the view has no allocation of its own: {linears:?}"
        );
    ...
    ///
    /// A physical replacement names a resource the guest deletes before the
    /// replacement reaches the head of its channel. The resource does not come
    /// back, so this is a decision and not a wait -- but it used to be
    /// classified as retryable, so the transaction was re-offered on every
    /// tick, its completion stamp was never published, and every later
    /// transaction on that channel refused behind it. The guest then sends
    /// nothing further and the census reads healthy, because nothing is
    /// refused and nothing is blocked.
    #[test]
    fn a_refused_lifecycle_command_publishes_its_refusal() {
        ...
        let admitted = runtime
            .admit_task_physical_replacement(
                channel,
                Box::default(),
                None,
                task,
                reims_vgpu_protocol::ObjectTableRef::new(42),
            )
            .unwrap();
        let transaction = admitted.transaction();

        // The guest deletes the surface, and the plane view goes with it.
        runtime.release_task_resources(task).unwrap();
    ...
    /// A surface-backing delete releases whatever hangs off the surface when
    /// it applies, not what hung off it when the guest sent the packet.
    ///
    /// The two are not the same: a delete waits for the head of its channel,
    /// and a plane view declared in between belongs to the surface the guest
    /// is deleting. Carrying the set from admission made the command describe
    /// a tree that no longer existed, and the owner refused it -- which cost
    /// the guest the delete, leaked every backing under the surface, and left
    /// the object names bound to resources nothing would ever release.
    #[test]
    fn a_tree_delete_releases_the_tree_the_surface_has_when_it_applies() {
        use crate::runtime::host::HostMemory;
        use reims_vgpu_paging::geometry::{DIRECTORY_DEPTH, DIRECTORY_ROOT_PFN};
        ...
        // The guest declares another view of the same plane while the delete
        // waits for the head of its channel.
        crate::runtime::replacement_object_lifecycle::apply_replacement_iosurface_plane_view(
            &mut runtime,
            task,
            43,
        ...
        assert!(
            matches!(progress, Some(ReplacementCpuProgress::Published { .. })),
            "the delete applies against the tree the surface has now: {progress:?}"
        );
        assert!(
            runtime
                .resolve_resource(task, reims_vgpu_protocol::ObjectTableRef::new(43))
                .is_none(),
            "the view declared after admission belongs to the tree the guest deleted"
        );
        assert!(runtime
            .resolve_resource(
                task,
