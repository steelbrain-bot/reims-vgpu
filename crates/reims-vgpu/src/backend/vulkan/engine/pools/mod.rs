...
fn slab_retain_enabled() -> bool {
    ...
    *ON.get_or_init(|| {
        let (state, value) = crate::config::read(crate::config::SLAB_RETAIN);
...
/// narrowable from the environment — see [`crate::config::BATCH_DRAWS`] for why
...
fn batch_max_draws(topology: crate::backend::vulkan::caps::memory_topology::MemoryTopology) -> u64 {
    ...
    let cap = match crate::config::count(crate::config::BATCH_DRAWS, default) {
        crate::config::Count::Narrowed(n) => n,
        crate::config::Count::Unset => default,
...
mod idle_slab_trim_tests {
    ...
    ///
    /// It cannot reach the `off` arm of [`crate::config::SLAB_RETAIN`], which
