...
impl reims_vgpu_observe::Decline for ImageFormatSpecializeError {
...
impl reims_vgpu_observe::Decline for UnclassifiedBinding {
    fn slug(&self) -> &'static str {
...
/// Whether [`reims_vgpu_config::UNUSED_BINDS`] is switched off, read once per process.
///
...
fn unused_binds_disabled() -> bool {
    ...
    *OFF.get_or_init(|| {
        let (state, value) = reims_vgpu_config::read(reims_vgpu_config::UNUSED_BINDS);
        match state {
            reims_vgpu_config::Switch::Off => {
                reims_vgpu_observe::off("unused_binds reason=unused_binds_disabled_by_env");
                true
            }
            reims_vgpu_config::Switch::Unrecognized => {
                reims_vgpu_observe::fail(format!(
            ...
            reims_vgpu_config::Switch::Unset | reims_vgpu_config::Switch::On => false,
        }
    })
}
...
mod more_tests {
    ...
    fn a_load_that_disagrees_with_its_pointee_is_refused_by_name() {
        ...
        assert_eq!(
            reims_vgpu_observe::Decline::slug(&verify_load_types(&words).unwrap_err()),
