...
pub fn note_draw(note: DrawNote) {
    crate::telemetry::note_route(frag_words_band(note.frag_words));
    ...
    if let Err(at) = trail.seen_pipes.binary_search(&note.pipeline_ref) {
        if trail.seen_pipes.len() >= SEEN_PIPES_MAX {
            crate::telemetry::note_route("pipe_firsts_full");
        } else {
...
pub fn note_submit(slot: usize, timeline: Option<u64>) {
    ...
    if slot >= trail.submits.len() {
        ...
        drop(trail);
        crate::telemetry::note_route("hang_submit_slot_over_capacity");
        return;
    }
